...
// const MOCK_PASSWORD = "staffmoeysedtech2025";
// const TOKEN_URL = `${API_BASE}/api/token/`;
// const API_TIMEOUT = 100000; // 5 seconds timeout
...
// import preahSihanoukStudentsData from "@/data/students/PreahSihanouk.json";  // Note: visible in folder but not in original code
...
// import preahSihanoukSchoolData from "@/data/schools/school-presihanuk.json";  // Note: visible in folder but not in original code
...
//   grades?: string[]; // Add grades field to School interface
// }
...
//   district?: string;
//   fullName?: string;
//   student_type?: string; // Add student_type field
// }
...
//   ...(kohKongStudentsData as Student[]), // Fixed: was kohkongStudentsData
//   ...(kampongChamStudentsData as Student[]), // Fixed: was kompongchamStudentsData
//   ...(kampongChhnangStudentsData as Student[]), // Fixed: was kompongchhnangStudentsData
//   ...(kampongSpeuStudentsData as Student[]), // Fixed: was kompongspeuStudentsData
//   ...(kampongThomStudentsData as Student[]), // Fixed: was kompongThomStudentsData
//   ...(kampotStudentsData as Student[]), // Fixed: was kompotStudentsData
//   ...(kandalStudentsData as Student[]),
//   ...(kratieStudentsData as Student[]),
//   ...(mondulKiriStudentsData as Student[]), // Fixed: was mondolkiriStudentsData
//   ...(oddarMeancheyStudentsData as Student[]),
//   ...(pailinStudentsData as Student[]),
//   ...(phnomPenhStudentsData as Student[]), // Fixed: was phnompenhStudentsData
//   ...(preahSihanoukStudentsData as Student[]),  // Note: visible in folder but not in original code
//   ...(preahVihearStudentsData as Student[]),
//   ...(preyVengStudentsData as Student[]), // Fixed: was preyvengStudentsData
//   ...(pursatStudentsData as Student[]),
//   ...(ratanakiriStudentsData as Student[]), // Fixed: was rattanakiriStudentsData
//   ...(siemReapStudentsData as Student[]), // Fixed: was siemreapStudentsData
//   ...(stungTrengStudentsData as Student[]), // Fixed: was stungtrengStudentsData
//   ...(svayRiengStudentsData as Student[]), // Fixed: was svayriengStudentsData
//   ...(takeoStudentsData as Student[]), // Fixed: was takaeoStudentsData
...
//   ...(preahSihanoukSchoolData as School[]),  // Note: visible in folder but not in original code
...
//   // --- API with Timeout and Fallback to Static Data ---
//   const fetchWithTimeout = useCallback(
//     async (
//       url: string,
//       stepName: string,
//       staticDataFallback: () => void
//     ): Promise<any[]> => {
...
//           console.log(`API timeout for ${stepName}, switching to static data`);
...
//     // NEW VALIDATION: Check student_type for grades 11 and 12
//     if ((selectedGrade === "11" || selectedGrade === "12") && usingStaticData) {
//       if (!selectedStudent.student_type || selectedStudent.student_type === "") {
//         setError("សិស្សថ្នាក់ 11 ឬ 12 ត្រូវបានបញ្ជាក់ប្រភេទវិទ្យាសាស្រ្ដ ឬ វិទ្យាសាស្រ្ដសង្គម ទើបអាចកូដប្រឡង។");
//         return;
//       }
//     }
...
//         } catch (err: any) {
//           if (err.message === "API timeout") {
//             console.log("API timeout for exam code, switching to static data");
//             setUsingStaticData(true);
            
//             // Fallback to static data approach - CHANGED FORMAT: firstName.lastName instead of lastName.firstName
//             const schoolId = selectedSchoolId;
//             const studentId = selectedStudent.student_ID || "00000";
//             const firstName = (selectedStudent.first_name || "").replace(/\s+/g, "");
//             const lastName = (selectedStudent.last_name || "").replace(/\s+/g, "");
...
//     setLoadingStep("ស្រុក");
//     setError(null);
    
//     const endpoint = `${API_BASE}/api/Base/data/v1/api/lookup/v1/district/${selectedProvinceId}/`;
    
//     fetchWithTimeout(endpoint, "ស្រុក", fetchDistrictsFromStatic)
//       .then((data) => {
//         if (data.length > 0) {
//           const districtNames = data.map((d) => d.district_name).filter(Boolean);
//           setDistricts(districtNames);
...
//       })
//       .catch((err: any) => {
//         setError(`បរាជ័យក្នុងការផ្ទុកស្រុក: ${err.message}`);
//         setDistricts([]);
//       })
//       .finally(() => {
//         setLoadingStep(null);
//       });
//   }, [selectedProvinceId, fetchWithTimeout, fetchDistrictsFromStatic]);

//   // 2. Fetch Schools
//   useEffect(() => {
//     if (!selectedProvinceId || !selectedDistrict) return;
    
//     setLoadingStep("សាលារៀន");
//     setError(null);
    
//     const encodedDistrict = encodeURIComponent(selectedDistrict);
//     const endpoint = `${API_BASE}/api/Base/data/v1/api/lookup/v1/school/${selectedProvinceId}/${encodedDistrict}/`;
    
//     fetchWithTimeout(endpoint, "សាលារៀន", fetchSchoolsFromStatic)
//       .then((data) => {
//         if (data.length > 0) {
//           const mappedSchools = data.map((s) => ({
//             id: s.geip_school_ID,
...
//       })
//       .catch((err: any) => {
//         setError(`បរាជ័យក្នុងការផ្ទុកសាលារៀន: ${err.message}`);
//         setSchools([]);
//       })
//       .finally(() => {
//         setLoadingStep(null);
//       });
//   }, [selectedProvinceId, selectedDistrict, fetchWithTimeout, fetchSchoolsFromStatic]);

//   // 3. Fetch Grades
//   useEffect(() => {
//     if (!selectedProvinceId || !selectedDistrict || !selectedSchoolId) return;
    
//     setLoadingStep("ថ្នាក់");
//     setError(null);
    
//     const encodedDistrict = encodeURIComponent(selectedDistrict);
//     const endpoint = `${API_BASE}/api/Base/data/v1/api/lookup/v1/grade/${selectedProvinceId}/${encodedDistrict}/${selectedSchoolId}/`;
    
//     fetchWithTimeout(endpoint, "ថ្នាក់", fetchGradesFromStatic)
//       .then((data) => {
//         if (data.length > 0) {
//           const gradeOptions = data.map((g) => String(g.grade || "")).filter(Boolean);
//           setGrades(gradeOptions);
...
//       })
//       .catch((err: any) => {
//         setError(`បរាជ័យក្នុងការផ្ទុកថ្នាក់: ${err.message}`);
//         setGrades([]);
//       })
//       .finally(() => {
//         setLoadingStep(null);
//       });
//   }, [selectedProvinceId, selectedDistrict, selectedSchoolId, fetchWithTimeout, fetchGradesFromStatic]);
...
//     setLoadingStep(`សិស្សថ្នាក់ ${selectedGrade}`);
//     setError(null);
    
//     const encodedDistrict = encodeURIComponent(selectedDistrict);
//     const endpoint = `${API_BASE}/api/Base/data/v1/api/lookup/v1/student/${selectedProvinceId}/${encodedDistrict}/${selectedSchoolId}/${selectedGrade}/`;
    
//     fetchWithTimeout(endpoint, `សិស្សថ្នាក់ ${selectedGrade}`, fetchStudentsFromStatic)
//       .then((data) => {
//         if (data.length > 0) {
//           const mappedStudents = data.map((s) => ({
//             ...s,
...
//       })
//       .catch((err: any) => {
//         setError(`បរាជ័យក្នុងការផ្ទុកសិស្ស: ${err.message}`);
//         setStudents([]);
//       })
//       .finally(() => {
//         setLoadingStep(null);
